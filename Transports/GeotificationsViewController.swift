...
let kSavedItemsKey = "savedItems"
 ...
 class GeotificationsViewController: UIViewController, AddGeotificationsViewControllerDelegate, MKMapViewDelegate, CLLocationManagerDelegate, ADBannerViewDelegate {
  ...
  func updateGeotificationsCount() {
    title = "Endroits (\(geotifications.count))"
  ...
  func startMonitoringGeotification(geotification: Geotification) {
    ...
    if !CLLocationManager.isMonitoringAvailableForClass(CLCircularRegion) {
      showSimpleAlertWithTitle("Désolés", message: "La sauvegarde de lieux n'est pas disponible pour cette appareil", viewController: self)
      return
    }
    // 2
    ...
    if CLLocationManager.authorizationStatus() != .AuthorizedAlways {
        showSimpleAlertWithTitle("Attention", message: "Votre endroit est sauvgardé mais il ne sera activé que lorsque vous aurez autorisé l'application à accéder à votre localisation. Allez dans Réglages >> Confidentialité >> Service de Localisation >> Transports. Appuyez ensuite sur le bouton \"Toujours\" pour activer cette fonctionnalité", viewController: self)
    }
    // 3
  ...
  func locationManager(manager: CLLocationManager!, monitoringDidFailForRegion region: CLRegion!, withError error: NSError!) {
    print("Erreur avec l'identifiant: \(region.identifier)")
  }
  
  func locationManager(manager: CLLocationManager!, didFailWithError error: NSError!) {
    print("La localisation a rencontré une erreur avec l'identifiant: \(error)")
